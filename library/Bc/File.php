...
class Bc_File
{
	public function Url($fid, $short=false) {
		...
		if (!$url) {
			...
			if ($file) {
				$url = ($short===true ? '' : 'http://'.$_SERVER['HTTP_HOST'].'/').'files/'.substr($fid, 0, 1).'/'.substr($fid, 1, 1).'/'.$fid.'.'.$file->Ext;
				$cacher->set($key, $url);
			}
		}
	...
	{
		$rows = &Bc_Db::t('attachment')->getByHash($hash);
